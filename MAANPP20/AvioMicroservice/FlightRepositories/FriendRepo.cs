...
namespace AvioMicroservice.FlightRepositories
{
    public class FriendRepo : IFriend
    {
        ...
        public async Task<ActionResult<IEnumerable<User>>> GetFriendRequestUsers(MAANPP20ContextFlight _context, string email)
        {
            var user = await _context.Users.Where(x => x.deleted == false)
                .FirstOrDefaultAsync(i => i.Email == email);
        ...
        private bool FriendRequestExists(MAANPP20ContextFlight _context, string myId, string hisId) =>
